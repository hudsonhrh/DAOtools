...
export async function submitRequest(prompt) {
    ...
    async function main() {
        ...
        const signerPrivateKey = process.env.NEXT_PUBLIC_PRIVATE_KEY
        const signer = new ethers.Wallet(signerPrivateKey, provider)
    
        // Consumer contract
        const consumerAddress = '0x02f4103C3F7ee662b5548b263a27a1a0a5BD6bBb'
        const consumerAbiPath = './artifacts/contracts/FunctionsConsumer.sol/FunctionsConsumer.json'
        const contractAbi = JSON.parse(await fetch(consumerAbiPath, 'utf8')).abi
        ...
        const subscriptionId = 563
        // Gas limit for the Chainlink Functions request 
        const requestGas = 5500000 
      
    
        // Default example
        const source = await fetch('./Functions-request-source.js').then((response) => response.text());
        const args = ["I am testing a project using you, only send the response 11 without a period or new line"]
